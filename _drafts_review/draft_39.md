...
*(원문의 실습 구조/코드는 그대로 두고, "왜 @Scheduled가 아니라 Quartz인가"와
Quartz를 실제로 쓸 때 마주치는 함정을 추가했습니다 — 검토 부탁드려요)*
...
핵심 차이는 "Job이 코드 배포와 묶여 있는가, 런타임에 독립적으로 관리되는가"다. 이
프로젝트에서는 반복 횟수 관리, TriggerListener를 통한 실시간 상태 갱신처럼 **런타임에
Job의 생애주기를 제어**해야 했기 때문에 Quartz를 선택했다.
...
원문에는 없지만 Quartz를 실제로 운영할 때 반드시 확인해야 하는 지점들:
...
- **클러스터 환경에서의 중복 실행**: 인스턴스를 여러 대 띄우면, 별도 설정 없이는
  ...
  락으로 "한 번만 실행됨"을 보장해야 한다 — 이 글의 예제는 인메모리 `RAMJobStore`
  기준이라 애플리케이션을 재시작하면 등록된 Job이 전부 사라진다는 한계도 있다.
